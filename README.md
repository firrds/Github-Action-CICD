# GitHub Actions と GitHub Pages を使った CI/CD 体験教材

## 0. この教材のねらい

この教材では、**Node.js を使って簡単な Web アプリを作成し、GitHub Actions で自動テスト・自動ビルドを行い、GitHub Pages に自動デプロイする**ところまでを体験します。

対象者は、**Git と GitHub の基本操作を理解している大学 2 年生**です。
...
VS Code の Live Server を使ってもよいですし、Windows 11 では `dist/index.html` をダブルクリックして既定のブラウザで開いても構いません。
...
### 23.8 `.github/workflows/deploy.yml`
...
### 課題 1
表示するメッセージを 3 種類用意し、ランダムに 1 つ表示されるように `build.mjs` を改良しなさい。

### 課題 2
`test/` にテストを追加し、メッセージ配列が空でないことを検証しなさい。

### 課題 3
`src/index.template.html` に「自分の学籍番号」「氏名」「最終更新日時」を表示しなさい。

### 課題 4
`main` ブランチ以外では deploy せず、テストだけ実行されるように workflow を改良しなさい。

### 課題 5
README に、自分の Pages URL と workflow の説明を追記しなさい。
...
### 25.2 Actions は成功したのに Pages が表示されない
...
## 24. 発展実験: プルリクエストによる GitHub Pages の共同編集

ここからは、ここまで作成した教材を土台として、**3〜4人グループで GitHub Pages を共同編集する実験**を行います。

この実験では、**グループリーダーの GitHub リポジトリ**を中心にして、各メンバーが自分の書きたいメッセージを **Pull Request (PR)** で提案し、リーダーがそれを確認して `main` へ取り込みます。`main` にマージされると GitHub Actions が再実行され、最終的に **全員のメッセージが GitHub Pages 上に表示されます**。Pull request は、変更を本番ブランチへ直接入れる前に、差分を確認・議論・レビューしてから統合する GitHub の中心的な共同開発機能です。 citeturn809580search11turn809580search1

### 24.1 この実験の学習目標

この実験では、次のことを体験します。

- fork して他人のリポジトリに変更提案を送る
- Pull Request を作成する
- リーダーが差分をレビューして merge する
- `main` への反映をきっかけに GitHub Actions が動く
- マージされた結果が GitHub Pages に自動で反映される

GitHub Actions の workflow は、`push` や `pull_request` などのイベントを契機に実行できます。今回の教材では Pages への公開は `main` への `push` を契機に動かしているため、**PR をマージして `main` が更新された時点で Pages 側が再デプロイされる**流れになります。 citeturn809580search14turn809580search2turn809580search10
...
GitHub Pages 上で、次のように複数人のメッセージが一覧で表示される状態を目指します。
...
これまでの教材では、アプリには 1 つのメッセージだけを表示していました。共同編集実験では、**複数人分のメッセージを配列として管理し、build 時に一覧表示へ変換する**ように変更します。

### 25.1 なぜこの変更が必要か

1 人だけのメッセージだと、メンバーが PR で追記しても共同編集の結果が見えにくくなります。そこで、**メッセージをデータとして別ファイルに切り出し、それを build で HTML に埋め込む構成**にします。

この構成にすると、メンバーは主に **`src/messages.json` を 1 行追加するだけ**で参加できるため、PR の練習に集中しやすくなります。

### 25.2 追加・変更するファイル

共同編集実験では、主に次のファイルを追加・変更します。
...
## 26. ステップ 18: `src/messages.json` を追加する

### 26.1 この操作の目的

全員のメッセージを 1 つのデータファイルで管理するためです。

### 26.2 `src/messages.json`
...
- `name`
  - 誰のメッセージかを表す
- `message`
  - GitHub Pages 上に表示したい本文

最初はリーダーのメッセージだけを入れておきます。後でメンバーが PR を使って、この配列に自分のデータを追加します。

---

## 27. ステップ 19: HTML テンプレートを共同編集版に変更する

### 27.1 この操作の目的

1 件のメッセージ表示ではなく、**複数メッセージの一覧表示**に変更します。

### 27.2 `src/index.template.html` の変更後の完成形
...
ここでは `{{MESSAGE}}` という 1 個の値ではなく、`{{MESSAGE_LIST}}` という **複数行の HTML** を埋め込むように変更しています。

---

## 28. ステップ 20: CSS を共同編集版に少し拡張する

### 28.1 `src/style.css` の追記内容
...
## 29. ステップ 21: build スクリプトを共同編集版に変更する

### 29.1 この操作の目的

`messages.json` を読み込み、メッセージ一覧の HTML を自動生成するためです。

### 29.2 `scripts/build.mjs` の変更後の完成形
...
メッセージ一覧データを JavaScript の配列として読み込んでいます。

#### 29.3.2 一覧表示用の HTML を作る
...
各メッセージを `<li>...</li>` に変換し、それを連結しています。

#### 29.3.3 `escapeHtml` を使う理由

PR では他の人が入力した文字列も取り込むため、`<` や `>` のような記号をそのまま埋め込むと表示崩れや意図しない HTML 解釈の原因になります。そこで、**表示用に安全な文字列へ変換**しています。

---

## 30. ステップ 22: テストを共同編集版に追加する

### 30.1 この操作の目的

PR で `messages.json` が壊れていないか、最低限の自動確認を行うためです。

### 30.2 `test/messages.test.mjs`
...
- 配列になっているか
- 少なくとも 1 件以上あるか
- `name` と `message` が空でないか

これにより、PR で JSON の形を壊してしまった場合に CI が失敗しやすくなります。

---

## 31. ステップ 23: リーダーが共同編集用の初期状態を push する

### 31.1 操作手順

リーダーはローカルで変更後、次を実行します。

```powershell
npm test
npm run build
git add .
git commit -m "Add collaborative message board feature"
git push
```
...
この時点で、共同編集の受け皿ができあがります。

---

## 32. ステップ 24: メンバーがリーダーのリポジトリを fork する

### 32.1 この操作の目的

メンバーはリーダーの `main` ブランチへ直接 push せず、**自分のコピー（fork）上で変更**してから PR を送ります。fork から PR を作成するのが、GitHub の代表的な共同編集モデルの 1 つです。

### 32.2 メンバーの操作手順

1. ブラウザでリーダーのリポジトリを開く
2. 右上の **Fork** をクリックする
...
```powershell
git clone https://github.com/YOUR_ACCOUNT/node-pages-cicd-sample.git
cd node-pages-cicd-sample
```

### 32.3 `upstream` を設定する

リーダーの元リポジトリを `upstream` として登録しておくと便利です。

```powershell
git remote add upstream https://github.com/LEADER_ACCOUNT/node-pages-cicd-sample.git
git remote -v
```
...
## 33. ステップ 25: メンバーが作業用ブランチを作る

### 33.1 この操作の目的

PR は通常、`main` に直接作業するのではなく、**自分専用の作業ブランチ**から送ります。これにより、変更内容を明確に切り分けられます。Pull request はブランチ上の変更提案をレビューして統合する仕組みです。 citeturn809580search11
...
## 34. ステップ 26: メンバーが `src/messages.json` に自分のメッセージを追加する

### 34.1 この操作の目的

自分が書きたいメッセージを、リーダーのサイトへ反映できる形で提案します。

### 34.2 追加例

元の `src/messages.json` が次の状態だとします。
...
## 35. ステップ 27: メンバーが commit して fork へ push する
...
- 変更を自分の作業ブランチに記録する
- GitHub 上の自分の fork に送る
- その branch を元に PR を作成できるようにする

---

## 36. ステップ 28: メンバーが Pull Request を作成する

### 36.1 操作手順

1. 自分の fork リポジトリを GitHub で開く
2. `Compare & pull request` が表示されたらクリックする
   - 表示されない場合は **Pull requests** → **New pull request** を選ぶ
3. 比較対象を確認する
   - **base repository**: リーダーのリポジトリ
   - **base branch**: `main`
   - **head repository**: 自分の fork
   - **compare branch**: `add-message-自分の名前`
4. タイトルを入力する
   - 例: `Add Tanaka message`
5. 本文に「追加したメッセージ」や「確認してほしい点」を書く
6. **Create pull request** をクリックする

fork から PR を作る場合は、変更先の base と、自分の fork 側の head を正しく選ぶことが重要です。GitHub Docs でも、fork から PR を作る手順では base branch と compare branch の確認が案内されています。 citeturn809580search1turn809580search3
...
## 37. ステップ 29: リーダーが Pull Request をレビューする

### 37.1 この操作の目的

リーダーは、メンバーの変更が正しいか確認してから統合します。これにより、**誰が何を変更したかを可視化しながら安全に共同編集**できます。
...
PR 上でもチェック結果を確認できる場合があります。今回の Pages デプロイ workflow は `push` を契機に本番公開しますが、授業設計によっては別途 `pull_request` 用の確認 workflow を追加することもできます。GitHub Actions は `pull_request` イベントでも起動できるため、PR 段階で自動テストだけを走らせる設計も可能です。 citeturn809580search14turn809580search8

### 37.4 merge の操作手順

1. リーダーのリポジトリの **Pull requests** を開く
2. 対象の PR をクリックする
3. 差分を確認する
4. 問題がなければ **Merge pull request** をクリックする
5. 必要に応じて **Confirm merge** をクリックする

GitHub では PR のマージ方法として通常の merge、squash、rebase などがありますが、授業ではまず標準的な merge で十分です。 citeturn809580search7turn809580search17

---

## 38. ステップ 30: GitHub Pages への反映を確認する

### 38.1 何が起こるか

PR が `main` にマージされると、リーダーのリポジトリで新しい `push` が発生します。その結果、これまで作成した `deploy.yml` が再実行され、`dist/` が再生成されて GitHub Pages が更新されます。Pages は branch から公開する方法もありますが、今回の教材では **GitHub Actions workflow を公開元にする構成**です。 citeturn809580search10turn809580search14

### 38.2 確認手順

1. リーダーのリポジトリで **Actions** タブを開く
2. 新しい workflow 実行が始まっていることを確認する
3. 成功したら GitHub Pages の URL を開く
4. マージしたメンバーのメッセージが一覧に増えていることを確認する

### 38.3 実験の完成形

メンバー全員分の PR をリーダーが順にマージすると、最終的に GitHub Pages には全員のメッセージが表示されます。
...
というように、**グループ全員が GitHub 上で共同編集した結果が公開ページに反映される**状態になります。

---

## 39. 共同編集実験で起こりやすいトラブル
...
対処:
- `messages.json` のカンマや `"` の閉じ忘れを確認する

### 39.2 同じ行を複数人が編集してコンフリクトする
...
対処:
- リーダーが順番に PR を取り込む
- メンバーは必要に応じて `upstream/main` を取り込み直してから再 push する
...
### 39.3 リーダーの Pages に反映されない

確認点:
- 本当にリーダーのリポジトリにマージしたか
- fork 側だけ更新していないか
- Actions が失敗していないか
...
## 40. この共同編集実験で理解してほしいこと

この実験の本質は、単にメッセージを 1 行追加することではありません。重要なのは次の流れです。
...
つまり、**共同開発・レビュー・CI/CD・公開** が 1 本の流れとしてつながっていることを体験するのが、この発展実験の狙いです。

---

## 41. 共同編集実験用の追加ソースコード一覧

ここでは、共同編集実験で追加・変更するファイルだけをまとめて再掲します。

### 41.1 `src/messages.json`
...
### 41.2 共同編集版 `src/index.template.html`
...
授業でこの実験を行う場合は、次のような順序にすると進めやすいです。

1. まず全員が基本版の CI/CD を 1 人で完成させる
2. 次に各グループでリーダーを 1 人決める
3. リーダーだけが共同編集版へアップデートする
4. メンバーは fork → branch → 編集 → PR を行う
5. リーダーが順番に PR をレビューして merge する
6. 最後に Pages を開き、全員のメッセージが並んだ状態を確認する

この順序にすると、**個人開発の CI/CD** と **チーム開発の PR ベースの運用** の違いが比較しやすくなります。
